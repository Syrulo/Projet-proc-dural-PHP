...
$db = Utils::connectDB();
$sql = $db->prepare("SELECT user.id, user.email, contact.firstname, contact.lastname, contact.address1, contact.address2, contact.city, contact.state, contact.zip, contact.message FROM user, contact WHERE user.id=$profil_id and contact.user_id=$profil_id");
$sql->execute();
...
if (isset($_POST['email']) && isset($_POST['firstname']) && isset($_POST['lastname']) && !empty($_POST['email']) && !empty($_POST['firstname']) && !empty($_POST['lastname'])) {
    ...
    $sql = $db->prepare("UPDATE `user` SET `email` = :email WHERE `user`.`id` = $profil_id");
            ...
            $sql->execute();
    $sql = $db->prepare("UPDATE `contact` SET `firstname` = :firstname, `lastname` = :lastname, `address1` = :address1, `address2` = :address2, `city` = :city,`state` = :state, `zip` = :zip, `message` = :message WHERE `contact`.`user_id` = $profil_id");
